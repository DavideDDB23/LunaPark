 ...
 * walk to it routing around every obstacle, pause 0.3–2 s, then choose a new one.
 *
 ...
 * engine below: direct bone rotations driven by trigonometric functions of a
 * phase variable, with pelvis rhythm, spine counter-rotation and arm swing.
 * ────────────────────────────────────────────────────────────────────────── */
 ...
 * animation in the project. The walk cycle uses direct bone rotations driven
 * by trigonometric functions of a phase variable (φ = 2π·phase):
 *
 *   1. PELVIS — vertical bob at 2× step frequency, lateral sway, yaw and roll
 *      all phase-locked to the gait cycle.
 *   2. LEGS — hip, knee and ankle flexion driven by sinusoidal functions of
 *      the phase, with right/left legs 180° out of phase. Knee uses a squared
 *      half-sine envelope to prevent hyperextension.
 *   3. ARMS — shoulder swing counter-phased to the hips, elbow flexion with
 *      a base offset and asymmetric flex envelope.
 *   4. SPINE & HEAD — counter-rotation against the pelvis, forward lean with
 *      speed, head stabilisation.
 *
 * Everything is phase-driven (phase advances by distance/cycleLen), so cadence
 * and ground speed always agree.
 * ────────────────────────────────────────────────────────────────────────── */
...
function makeGaitRig(fig) {
  ...
  const armBone = bones['Foot.L'].parent;
  const invArm = new THREE.Matrix4().copy(armBone.matrixWorld).invert();
  armBone.matrixWorld.decompose(_vBody, _qT, _ds);
  ...
  const L1 = hip0.distanceTo(knee0), L2 = knee0.distanceTo(ankle0);
  rig.legLen = (L1 + L2) * 2 / 2;
  ...
  for (const side of ['L', 'R']) {
    ...
    dq.copy(leg.flat).multiply(_qInv.copy(qFt0).invert());
    let minY = Infinity, found = false;
    fig.traverse((o) => {
      ...
      for (let i = 0; i < posA.count; i++) {
        ...
        vtx.fromBufferAttribute(posA, i).sub(ankle0).applyQuaternion(dq);
        if (vtx.y < minY) minY = vtx.y;
        found = true;
      }
    });
    if (found) { hSum += -minY; sides++; }
  }
  rig.ankleH = sides ? Math.max(0.01, hSum / sides) : 0.07 * rig.legLen;

  for (const name of ['UpperLeg.L', 'LowerLeg.L', 'Foot.L', 'UpperLeg.R', 'LowerLeg.R', 'Foot.R',
                      'Abdomen', 'Torso', 'Neck', 'Head', 'UpperArm.L', 'UpperArm.R', 'LowerArm.L', 'LowerArm.R']) {
    const b = bones[name];
    if (!b) continue;
    relTo(invArm, b, _vBody, _qT);
    _qInv.copy(_qT).invert();
    const target = name.includes('Leg') || name.includes('Foot') ? rig.legs : rig.upper;
    const side = name.includes('.L') ? 'L' : name.includes('.R') ? 'R' : null;
    const key = side ? side : name;
    target[key] = {
      ...target[key] || {},
...
function applyGaitPose(w, time) {
  ...
  const phi = p * TWO_PI;
  const thetaR = phi;
  const thetaL = phi + Math.PI;

  const bob = GAIT.A_bounce * Math.cos(2 * phi) * L * mb;
  const swayX = GAIT.A_sway * Math.sin(phi) * L * mb;
  const hipY = rig.ankleH + GAIT.hipHeight * L + bob + breathe * 0.004 * L * (1 - mb);
  const yaw = -GAIT.A_yaw * Math.sin(phi) * mb * vNorm;
  const roll = GAIT.A_roll * Math.cos(phi) * mb + w.lean;
  ...
  _vBody.set(rig.bodyPos0.x + swayX, hipY - rig.bodyPos0.y, rig.bodyPos0.z);
  rig.body.position.copy(_vBody);
  _qBodyInv.copy(_qBody).invert();

  for (let i = 0; i < 2; i++) {
    const theta = i === 0 ? thetaR : thetaL;
    const leg = i === 0 ? rig.legs.L : rig.legs.R;

    const hipFlex = GAIT.A_hip * Math.sin(theta) * mb;
    const kneeFlex = GAIT.A_knee * Math.pow((Math.sin(theta - Math.PI / 2) + 1) / 2, 2) * mb;
    const ankleFlex = GAIT.A_ankle * Math.sin(theta + Math.PI / 4) * mb;

    spinBone(leg.up, hipFlex, 0, leg.sideSign * GAIT.footOut * mb);
    spinBone(leg.lo, kneeFlex, 0, 0);
    spinBone(leg.ft, ankleFlex, 0, leg.sideSign * GAIT.footOut * mb, leg.flat);
  }

  const U = rig.upper;
  for (let i = 0; i < 2; i++) {
    const theta = i === 0 ? thetaR : thetaL;
    const shoulderKey = i === 0 ? 'UpperArm.L' : 'UpperArm.R';
    const elbowKey = i === 0 ? 'LowerArm.L' : 'LowerArm.R';

    const shoulderSwing = -GAIT.A_shoulder * Math.sin(theta) * mb;
    const elbowFlex = GAIT.C_base + GAIT.A_elbow * ((-Math.sin(theta) + 1) / 2) * mb;

    spinBone(U[shoulderKey], shoulderSwing, 0, 0);
    spinBone(U[elbowKey], -elbowFlex, 0, 0);
  }

  const spineYaw = -GAIT.A_yaw * Math.sin(phi) * mb;
  const spineRoll = -GAIT.A_roll * Math.cos(phi) * mb;
  ...
  spinBone(U['Abdomen'], lean * 0.45, spineYaw * 0.45, spineRoll * 0.5);
  spinBone(U['Torso'], lean * 0.55 + ID('Torso', 0),
           spineYaw * 0.55 + ID('Torso', 1), spineRoll * 0.35 + ID('Torso', 2));

  const headPitch = GAIT.A_head * Math.sin(2 * phi - Math.PI / 2) * mb;
  const headLook = ((1 - mb) * 0.45 * Math.sin(time * 0.4 + seed)
                   + mb * 0.10 * Math.sin(time * 0.6 + seed * 2)) * (idle ? 1 - iw : 1);

  spinBone(U['Neck'], -lean * 0.4 + headLook * 0.1 + ID('Neck', 0),
           -spineYaw * 0.2 + headLook * 0.35 + ID('Neck', 1), 0);
  spinBone(U['Head'], headPitch - lean * 0.3 + ID('Head', 0),
           -spineYaw * 0.3 + headLook + ID('Head', 1), ID('Head', 2));
}
...
export async function buildVisitors({
  ...
  for (let i = 0; i < count; i++) {
    ...
    const w = {
      root, rider, rig,
      state: 'waiting', wait: rand(0.3, 2.0),
      ...
      idleFn: null, idleW: 0, idleT: -1,
      cycleLen: rig ? GAIT.cycleLen * rig.scale * rand(0.95, 1.05) : 2.2,
    };
  ...
  group.userData.tick = (delta, time) => {
    ...
    for (const w of visitors) {
      ...
      if (w.state === 'waiting') {
        ...
        if (!tgt) { w.state = 'waiting'; w.wait = rand(0.3, 2.0); }
        else {
          _dir.set(tgt.x - w.root.position.x, 0, tgt.z - w.root.position.z);
          const dist = _dir.length();
          if (dist < 0.4) { if (++w.pathIdx >= w.path.length) { w.state = 'waiting'; w.wait = rand(0.3, 2.0); } }
          else {
            _dir.multiplyScalar(1 / dist);
            const arrive = w.pathIdx >= w.path.length - 1 ? clamp(dist / 0.8, 0.5, 1) : 1;
            const eff = w.speed * (0.5 + 0.5 * smooth01(w.moveBlend)) * arrive;
            ...
            const cycleWorld = w.cycleLen || 2.2;
            w.phase = (w.phase + (step / cycleWorld)) % 1;
          }
        }
      }

      const targetBlend = w.state === 'walking' ? 1 : 0;
      w.moveBlend += (targetBlend - w.moveBlend) * clamp(dt * 7, 0, 1);
